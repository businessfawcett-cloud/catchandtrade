...
// Main grading route - handles multiple endpoints
...
export async function POST(request: NextRequest) {
  ...
  // Handle /api/grading/calculate POST
  if (url.includes('/calculate')) {
    return handleCalculatePost(request);
  }
  
  return handleCalculate(request);
}

async function handleCalculate(request: NextRequest) {
  ...
  return calculateGradeROI(cardValue, company, tier, expectedGrade);
}

async function handleCalculatePost(request: NextRequest) {
  try {
    const body = await request.json();
    const cardValue = parseFloat(body.cardValue || body.rawPrice || '0');
...
function calculateGradeROI(cardValue: number, company: string, tier: string, expectedGrade: number) {
